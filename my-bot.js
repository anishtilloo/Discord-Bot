...
client.on("ready", () => {
  ...
  const attachment = new Discord.MessageAttachment(
    "https://cdnb.artstation.com/p/assets/images/images/035/214/141/large/marcos-emilio-trafalgar-law-2.jpg?1614376282"
  );
...
client.on("messageCreate", (recivedMessage) => {
  ...
  recivedMessage.channel.send(
    "Message Recived, " +
      recivedMessage.author.toString() +
      ": " +
...
function helpCommand(arguments, recivedMessage) {
  if (arguments.length === 0) {
    recivedMessage.channel.send(
      "I am not sure what you need help with. Try `!help [topics]`"
    );
  } else {
    recivedMessage.channel.send(
